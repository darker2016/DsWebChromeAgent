...
**引导包装（v0.2.0+）**
- [ ] 注入一个专家团：输入框以「我将使用下面的「…」专家团…」开头，**含「运行环境说明」（忽略工具/脚本/子任务指令）**，依次含主理人 + 各成员定义，结尾等待用户输入任务
- [ ] 注入一个单体技能：输入框以「你正在启用…技能…严格按定义执行」开头
- [ ] 注入扁平结构组（如 video-gen-skills）：lead / 成员路径正确，不报「载入失败」
- [ ] 工具依赖重的组（如 aicoding-architecture、invoice-verify）：AI 按运行环境说明在对话内扮演，不试图调用脚本/工具
