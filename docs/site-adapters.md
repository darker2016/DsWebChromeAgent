...
- **注入内容 = 引导模板 + 技能正文**（`DSWA.GUIDE`，见 `src/shared/constants.js`）。
  - group 引导：「我将使用下面的「…」专家团来执行后续的任务」→ **运行环境说明** → 主理人定义 → 各成员定义 → 结尾等待用户输入任务；注入**主理人 + 全部成员**的 SKILL.md 正文。
  - single 引导：「你正在启用「…」技能…严格按定义执行」+ **运行环境说明** + 自身 SKILL.md 正文。
- **运行环境说明（关键）**：网页对话没有工具/脚本/文件系统/子代理，引导里明确要求 AI **忽略技能定义中的「调用工具、创建团队、派发子任务、运行脚本、写文件、外部数据源」指令**，改为在对话内依次扮演成员、基于当前对话内容完成。多数专家团/单体技能为 Claude Code 等环境编写，此说明是它们能在网页对话工作的关键。
