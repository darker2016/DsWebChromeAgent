...
name: lark-base
version: 1.2.3
description: "飞书多维表格（Base）操作：建表、字段、记录、视图、统计、公式/lookup、表单、仪表盘、workflow、角色权限；遇到 Base/多维表格/bitable 或 /base/ 链接时使用。文件导入/导出转 lark-drive。"
metadata:
  requires:
    bins: ["lark-cli"]
...
- Base 文档只保留会影响 Base 路径选择的权限规则；认证、scope、权限不足恢复由 agent 平台处理。
...
- 默认显式使用 `--as user` 操作用户资源。
- user 身份报 scope/授权不足，或错误中包含 `missing_scopes` / `hint`，由 agent 平台为当前用户补开相应 scope，不要盲目重试。
- `91403` 或明确不可访问错误不要盲目重试。
- `+base-create` / `+base-copy` 执行后关注返回中的 `permission_grant`，并把用户是否可打开新 Base 告知用户。
...
| `91403` | 无权限访问该 Base，由 agent 平台补齐权限或引导用户解决，不要盲目重试 |
